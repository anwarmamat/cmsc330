...
co_star(X,Y):- actor(M, X, _), actor(M, Y, _), X \= Y.
co_star(X,Y):-actor(M, X, _), actress(M, Y, _),X \= Y.
co_star(X,Y):-actress(M, X, _), actress(M, Y, _),X \= Y.
co_star(X,Y):-actress(M, X, _), actor(M, Y, _),X \= Y.
...
co_star(M,X,Y) :- actor(M, X,_), actor(M, Y,_),X\=Y.
co_star(M,X,Y) :- actress(M, X,_), actress(M, Y,_),X\=Y.
co_star(M,X,Y) :- actor(M, X,_), actress(M, Y,_),X\=Y.
co_star(M,X,Y) :- actress(M, X,_), actor(M, Y,_),X\=Y.
...
path(X, Y):- path(X, Y, [X]).
path(X, X, V):- write(V).
path(X, Z, Visited):- co_star(_,X,Y), (\+ member(Y,Visited)), length(Visited,Len), Len < 5,path(Y,Z,[Y|Visited]).
...
co_star(X,Y):-actress(M, X, _), actor(M, Y, _),X \= Y.
 ...
 f(M,A):- actor(M, A, _), director(M, A).